...
export const PullToRefresh: React.FC<PullToRefreshProps> = ({ onRefresh, children, pullY }) => {
    ...
    // Use MotionValues for smooth 60fps animations without React renders
    const pullProgress = useTransform(y, (latest: number) => Math.min(1, Math.max(0, latest / PULL_THRESHOLD)));
    const rotate = useTransform(pullProgress, [0, 1], [0, 540]);
    ...
    useEffect(() => {
        ...
        // Only attach non-passive listener if we are at the top and can drag.
        // This prevents scroll blocking/jank when scrolling down the list.
        if (canDrag) {
